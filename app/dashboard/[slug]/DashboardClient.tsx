...
export function DashboardClient({ trip: initialTrip, stages: initialStages, participants, participations: initialP, slug }: any) {
  ...
  async function saveTrip() {
    await supabase.from("roadtrips").update({
      title: trip.title,
  ...
  async function addStage(e: React.FormEvent) {
    ...
    const { data } = await supabase.from("stages").insert({
      roadtrip_id: trip.id, name: newStage.name, description: newStage.description,
      date_start: newStage.date_start || null, date_end: newStage.date_end || null,
      lat, lng, order_index: stages.length
    }).select().single()
  ...
  const tabStyle = (tab: string) => ({
    ...
    background: activeTab === tab ? "#8fb840" : "#141a0e",
    color: activeTab === tab ? "#0e1409" : "#7a8a6a",
  })

  const inputStyle = { background: "#0e1409", border: "1px solid #2d4a1e", color: "#e8e4d9", borderRadius: "10px", padding: "10px 14px", width: "100%", fontSize: "14px" }
    ...
    <div>
      ...
      <div style={{background: "#141a0e", borderRadius: "16px", padding: "20px", marginBottom: "20px", border: "1px solid #1a2212"}}>
        ...
        {editingTrip ? (
          <div style={{display: "flex", flexDirection: "column", gap: "10px"}}>
            <div>
              <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>NOM DU TRIP</label>
              <input style={inputStyle} value={trip.title} onChange={e => setTrip({...trip, title: e.target.value})} />
            </div>
            <div>
              <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>DESCRIPTION</label>
              <textarea style={{...inputStyle, minHeight: "80px", resize: "vertical"}} value={trip.description || ""} onChange={e => setTrip({...trip, description: e.target.value})} />
            </div>
            <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px"}}>
              <div>
                <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>DATE DÉBUT</label>
                <input type="date" style={inputStyle} value={trip.date_start || ""} onChange={e => setTrip({...trip, date_start: e.target.value})} />
              </div>
              <div>
                <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>DATE FIN</label>
                <input type="date" style={inputStyle} value={trip.date_end || ""} onChange={e => setTrip({...trip, date_end: e.target.value})} />
              </div>
            </div>
          </div>
        ) : (
      ...
      {activeTab === "participations" && (
        <div>
                ...
                {pending.map((p: any) => (
                  <div key={p.id} style={{background: "#1a1200", border: "1px solid #5c3d2e", borderRadius: "14px", padding: "14px 16px", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap"}}>
                    ...
                    <div style={{flex: 1, minWidth: "120px"}}>
                      ...
                      <div style={{color: "#7a8a6a", fontSize: "12px"}}>
                    ...
                    <div style={{display: "flex", gap: "8px"}}>
                      ...
                      <button onClick={() => updateStatus(p.id, "rejected")} disabled={loadingP === p.id}
                        style={{background: "#2d0000", color: "#f87171", padding: "8px 16px", borderRadius: "100px", border: "none", cursor: "pointer", fontWeight: "700", fontSize: "13px"}}>
                        ✕
                      </button>
                    </div>
                  </div>
          ...
          <div>
            <h3 style={{fontSize: "13px", color: "#7a8a6a", letterSpacing: "1px", marginBottom: "12px"}}>CONFIRMÉS ({approved.length})</h3>
            {approved.length === 0 ? (
              ...
              <div style={{display: "flex", flexDirection: "column", gap: "8px"}}>
                {approved.map((p: any) => (
                  <div key={p.id} style={{background: "#0d2d1a", border: "1px solid #2d4a1e", borderRadius: "14px", padding: "12px 16px", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap"}}>
                    <div style={{width: "32px", height: "32px", borderRadius: "50%", background: "#2d4a1e", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "700", fontSize: "13px", flexShrink: 0}}>
                      {p.participants?.name?.[0]?.toUpperCase()}
                    </div>
                    <div style={{flex: 1, minWidth: "120px"}}>
                      <div style={{fontWeight: "600", fontSize: "14px"}}>{p.participants?.name}</div>
                      <div style={{color: "#7a8a6a", fontSize: "12px"}}>
                        📍 {p.stages?.name}
                        {p.overlap_start && <span style={{color: "#8fb840"}}> · ensemble {p.overlap_start} → {p.overlap_end}</span>}
                      </div>
                    </div>
                    <button onClick={() => deleteParticipation(p.id)}
                      style={{background: "transparent", color: "#4a5a3a", border: "none", cursor: "pointer", fontSize: "18px", padding: "4px"}}>×</button>
                  </div>
                ))}
              </div>
            )}
          ...
          {addingStage && (
            <form onSubmit={addStage} style={{background: "#141a0e", border: "1px solid #2d4a1e", borderRadius: "16px", padding: "20px", marginBottom: "16px"}}>
              ...
              <div style={{display: "flex", flexDirection: "column", gap: "10px"}}>
                ...
                <button type="submit" style={{background: "#8fb840", color: "#0e1409", border: "none", borderRadius: "100px", padding: "12px", fontSize: "14px", fontWeight: "700", cursor: "pointer"}}>
                  Ajouter
                </button>
              </div>
            </form>
            ...
            {stages.map((s: any, i: number) => (
              <div key={s.id} style={{background: "#141a0e", border: "1px solid #1a2212", borderRadius: "16px", padding: "16px"}}>
                <div style={{display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: editingStage === s.id ? "14px" : "0"}}>
                  ...
                  <div style={{display: "flex", gap: "8px"}}>
                    ...
                    <button onClick={() => deleteStage(s.id)}
                      style={{background: "#2d0000", color: "#f87171", border: "none", borderRadius: "100px", padding: "5px 12px", fontSize: "12px", cursor: "pointer"}}>
                      🗑
                    </button>
                  </div>
                </div>
                {editingStage === s.id ? (
                  <div style={{display: "flex", flexDirection: "column", gap: "10px"}}>
                    <div>
                      <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>VILLE</label>
                      <input style={inputStyle} value={s.name} onChange={e => updateStageLocal(s.id, "name", e.target.value)} />
                    </div>
                    <div>
                      <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>DESCRIPTION</label>
                      <textarea style={{...inputStyle, minHeight: "60px", resize: "vertical"}} value={s.description || ""} onChange={e => updateStageLocal(s.id, "description", e.target.value)} />
                    </div>
                    <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px"}}>
                      <div>
                        <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>ARRIVÉE</label>
                        <input type="date" style={inputStyle} value={s.date_start || ""} onChange={e => updateStageLocal(s.id, "date_start", e.target.value)} />
                      </div>
                      <div>
                        <label style={{fontSize: "11px", color: "#7a8a6a", letterSpacing: "1px", display: "block", marginBottom: "5px"}}>DÉPART</label>
                        <input type="date" style={inputStyle} value={s.date_end || ""} onChange={e => updateStageLocal(s.id, "date_end", e.target.value)} />
                      </div>
                    </div>
                  </div>
                ) : (
      ...
      {activeTab === "messages" && (
        <div>
          <div style={{display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap"}}>
            {stages.map((s: any) => (
              <button key={s.id} onClick={() => loadMessages(s.id)}
                style={{padding: "8px 16px", borderRadius: "100px", fontSize: "13px", fontWeight: "600", cursor: "pointer", border: "none",
                  background: activeStageMsg === s.id ? "#2d4a1e" : "#141a0e",
                  color: activeStageMsg === s.id ? "#8fb840" : "#7a8a6a",
                  borderWidth: "1px", borderStyle: "solid", borderColor: activeStageMsg === s.id ? "#3d6429" : "#1a2212"}}>
                📍 {s.name}
              </button>
            ))}
          </div>
          ...
          {!activeStageMsg && (
            <div style={{background: "#141a0e", borderRadius: "16px", padding: "40px", textAlign: "center", color: "#4a5a3a"}}>
              Sélectionne une étape pour voir sa messagerie
            </div>
          )}

          {activeStageMsg && (
            <div style={{background: "#141a0e", borderRadius: "16px", border: "1px solid #1a2212", overflow: "hidden"}}>
              <div style={{padding: "16px 20px", borderBottom: "1px solid #1a2212"}}>
                <h3 style={{fontSize: "14px", fontWeight: "700", color: "#8fb840"}}>
                  💬 {stages.find((s: any) => s.id === activeStageMsg)?.name}
                </h3>
              </div>
              <div style={{height: "320px", overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: "12px"}}>
                ...
                {!loadingMsg && messages.length === 0 && (
                  <div style={{textAlign: "center", color: "#4a5a3a", marginTop: "40px"}}>Aucun message pour cette étape</div>
                )}
                {messages.map((m: any) => (
                  <div key={m.id} style={{display: "flex", gap: "10px", alignItems: "flex-start"}}>
                    <div style={{width: "30px", height: "30px", borderRadius: "50%", background: "#2d4a1e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: "700", color: "#8fb840", flexShrink: 0}}>
                      {m.author[0].toUpperCase()}
                    </div>
                    <div style={{flex: 1}}>
                      <div style={{display: "flex", gap: "8px", alignItems: "center", marginBottom: "4px"}}>
                        <span style={{fontSize: "13px", fontWeight: "700"}}>{m.author}</span>
                        <span style={{fontSize: "11px", color: "#4a5a3a"}}>{new Date(m.created_at).toLocaleTimeString("fr-FR", {hour:"2-digit", minute:"2-digit"})}</span>
                      </div>
                      <div style={{background: "#0e1409", borderRadius: "10px", padding: "10px 14px", fontSize: "14px", color: "#e8e4d9", lineHeight: "1.5"}}>
                        {m.content}
                      </div>
                    </div>
                  </div>
              ...
              <form onSubmit={sendMessage} style={{padding: "16px", borderTop: "1px solid #1a2212", display: "flex", gap: "10px"}}>
                <input
                  value={newMsg}
                ...
                />
                <button type="submit"
                  style={{background: "#8fb840", color: "#0e1409", border: "none", borderRadius: "100px", padding: "10px 20px", fontSize: "14px", fontWeight: "700", cursor: "pointer", whiteSpace: "nowrap"}}>
                  Envoyer
                </button>
              </form>
            </div>
